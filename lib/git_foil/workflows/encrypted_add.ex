defmodule GitFoil.Workflows.EncryptedAdd do
  @moduledoc """
  ...
  alias GitFoil.Workflows.EncryptedAdd.Progress.{Noop, ProgressBar}
  ...
  @default_telemetry_prefix [:git_foil, :encrypted_add]
  ...
  ## Options
    ...
    * `:progress_adapter` — module that implements
      `GitFoil.Workflows.EncryptedAdd.Progress`. Defaults to a terminal
      progress bar; pass `GitFoil.Workflows.EncryptedAdd.Progress.Noop` to
      disable visuals.
    * `:progress_opts` — options forwarded to the progress adapter.
    * `:telemetry_prefix` — telemetry event prefix, defaults to
      `[:git_foil, :encrypted_add]`.
  """
