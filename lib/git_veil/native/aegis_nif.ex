defmodule GitVeil.Native.AegisNif do
  @moduledoc """
  ...
  **Security:**
  ...
  use Rustler, otp_app: :git_veil, crate: :aegis_nif
  ...
  ## Example
      ...
      iex> {:ok, {ct, tag}} = GitVeil.Native.AegisNif.encrypt(key, nonce, "secret", "metadata")
      iex> byte_size(tag)
      32
  """
  ...
  ## Example
      ...
      iex> nonce = :crypto.strong_rand_bytes(32)
      iex> {:ok, {ct, tag}} = GitVeil.Native.AegisNif.encrypt(key, nonce, "secret", "metadata")
      iex> {:ok, plaintext} = GitVeil.Native.AegisNif.decrypt(key, nonce, ct, tag, "metadata")
      iex> plaintext
      "secret"
  """
