defmodule GitVeil.CLITest do
  ...
  setup do
    ...
    {:ok, _} = start_supervised(GitVeil.Adapters.InMemoryKeyStorage)

    {:ok, keypair} = GitVeil.Adapters.InMemoryKeyStorage.generate_keypair()
    :ok = GitVeil.Adapters.InMemoryKeyStorage.save_keypair(keypair)
  ...
  describe "help command" do
    test "returns help text with no arguments" do
      {:ok, output} = CLI.run([])
      assert output =~ "GitVeil - Quantum-resistant Git encryption"
    end
    ...
    test "returns help text with -h flag" do
      {:ok, output} = CLI.run(["-h"])
      assert output =~ "git_veil <command>"
    end
  end
  ...
  describe "doctor command" do
    test "runs health checks" do
      {:ok, output} = CLI.run(["doctor"])
      assert output =~ "GitVeil Health Check"
      assert output =~ "Checks passed"
    end

    test "supports verbose flag" do
      {:ok, output} = CLI.run(["doctor", "--verbose"])
      assert output =~ "Details:"
      assert output =~ "erlang_crypto"
    end
  end
  ...
  describe "argument parsing" do
    ...
    test "parses doctor with verbose option" do
      {:ok, output} = CLI.run(["doctor", "--verbose"])
      assert output =~ "Details"
    end
  end
